...
if __name__ == "__main__":
    y = tf.random.normal([128,2,2,2])
    y1 = tf.norm(tf.reshape(y,[128,-1]),axis=-1,ord='euclidean')
    y2 = tf.norm(tf.reshape(y,[128,-1]),axis=-1,ord=2)
    y3 = tf.norm(tf.reshape(y,[32,32]),axis=(-2,-1),ord='fro')#矩阵范数必须指定两个维度
    y4 = tf.norm(tf.reshape(y,[128,-1]),axis=-1,ord=1)
    print(y1)
    print(y2)
    print(y3)
    print(y4)