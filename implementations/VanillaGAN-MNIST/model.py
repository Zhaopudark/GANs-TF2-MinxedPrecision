...
class VanillaGAN(tf.keras.Model):
    ...
    def train_step_D(self,trainX,trainY,step):
        with tf.GradientTape(persistent=True) as D_tape:
            ...
            e = tf.random.uniform([trainY.shape[0]],0.0,1.0)
    ...
    def train_step(self,trainX,trainY,step):
        ...
        self.D_optimizer.apply_gradients(zip(gradients_of_D,self.D.trainable_variables))
        self.G_optimizer.apply_gradients(zip(gradients_of_G,self.G.trainable_variables))
        return D_loss,G_loss,gradients_of_D[0],gradients_of_G[0]
        ...
        for _ in range(epoches):
            ...
            for trainX,trainY in self.train_set:
                ...
                if self.loss_name in ["WGAN","WGAN-GP"]:
                    for __ in range(5):
                        D_loss = self.train_step_D(trainX,trainY,tf.constant(step,shape=[1],dtype=tf.uint32))
                    for __ in range(1):
                        G_loss = self.train_step_G(trainX,trainY,tf.constant(step,shape=[1],dtype=tf.uint32))
                elif self.loss_name in ["Vanilla","LSGAN"]:
                    D_loss,G_loss,gd,gg= self.train_step(trainX,trainY,tf.constant(step,shape=[1],dtype=tf.uint32))
                else:
    ...
    def wirte_summary(self,step,seed,G,G_loss,D_loss,gd,gg,out_path):
        plt.figure(figsize=(10,10))#图片大一点才可以承载像素
        ...
        with self.train_summary_writer.as_default():
            ...
            tf.summary.scalar('gd',gd,step=step)
            tf.summary.scalar('gg',gg,step=step)
