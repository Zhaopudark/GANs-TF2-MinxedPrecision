...
tmp_path = "D:/Work/Codes_tmp/3DCycleGAN-mixed-test"
out_path = "D:/Work/Codes_tmp/3DCycleGAN-mixed-test/out"
...
def map_func(x):
    ...
    A = tf.reshape(x[:,0,:,:,:],[1,64,64,64,1], name=None)
    A = (A-0.0)/1
    B = tf.reshape(x[:,1,:,:,:],[1,64,64,64,1], name=None)
    B = (B-0.0)/1
    return A,B
...
num_threads = 4
dataset = train_dataset.DataPipeLine(train_path,target_size=[64,64,64])
...
test_set = train_dataset.DataPipeLine(test_path,target_size=[64,64,64])
...
model = model.CycleGAN(train_set=dataset,
                       ...
                       tmp_path=tmp_path,
                       out_path=out_path)
model.build(X_shape=[None,64,64,64,1],Y_shape=[None,64,64,64,1])
