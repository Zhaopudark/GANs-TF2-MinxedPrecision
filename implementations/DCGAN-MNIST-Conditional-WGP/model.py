...
global_input_Y_label_shape = [None,10]
################################################
class DCGAN(tf.keras.Model):
    ...
    def __init__(self,
        ...
        self.get_seed()
    def build(self,input_shape_G,input_shape_D,label_shape=global_input_Y_label_shape):
        """
        input_shape必须切片 因为在底层会被当做各层的输出shape而被改动
        """
        self.G.build(input_shape=input_shape_G[:],label_shape=label_shape)#G X->Y
        self.D.build(input_shape=input_shape_D[:],label_shape=label_shape)#D Y or not Y
        self.built = True
    def optimizers_config(self,mixed_precision=False,learning_rate=1e-4):
    ...
    @tf.function(input_signature=[tf.TensorSpec(shape=global_input_X_shape,dtype=tf.float32),\
                                  ...
                                  tf.TensorSpec(shape=global_input_Y_label_shape,dtype=tf.float32),\
                                  tf.TensorSpec(shape=[4],dtype=tf.int32),\
                                  tf.TensorSpec(shape=[1],dtype=tf.uint32)])
    def train_step_D(self,trainX,trainY,labelY,y_shape,step):
        with tf.GradientTape(persistent=True) as D_tape:
            GeneratedY = self.G(trainX,labelY)
            D_real_out = self.D(trainY,labelY)
            D_fake_out = self.D(GeneratedY,labelY)

            e = tf.random.uniform(shape=y_shape,minval=0.0,maxval=1.0)
            mid_Y = e*trainY+(1-e)*GeneratedY
            with tf.GradientTape() as GP:
                GP.watch(mid_Y)
                inner_loss = self.D(mid_Y,labelY)
    ...
    @tf.function(input_signature=[tf.TensorSpec(shape=global_input_X_shape,dtype=tf.float32),\
                                  ...
                                  tf.TensorSpec(shape=[4],dtype=tf.int32),\
                                  tf.TensorSpec(shape=[1],dtype=tf.uint32)])
    def train_step_G(self,trainX,trainY,labelY,y_shape,step):
        with tf.GradientTape(persistent=True) as G_tape:
            GeneratedY = self.G(trainX,labelY)
            # Dy_real_out = self.Dy(trainY,labelY)
            D_fake_out = self.D(GeneratedY,labelY)
    ...
    @tf.function(input_signature=[tf.TensorSpec(shape=global_input_X_shape,dtype=tf.float32),\
                                  ...
                                  tf.TensorSpec(shape=[4],dtype=tf.int32),\
                                  tf.TensorSpec(shape=[1],dtype=tf.uint32)])
    def train_step(self,trainX,trainY,labelY,y_shape,step):
    ...
    def train(self,epoches):
        ...
        for _ in range(epoches):
            start = time.time()
            for trainX,trainY,labelY in self.train_set:
    ...
    def test(self,take_nums):
        ...
        for take in range(take_nums):
            ...
            for i in range(100):
                ...
                label = i//10
                one_hot_label = tf.one_hot(label,depth=int(global_input_Y_label_shape[-1]),on_value=1.0,off_value=0.0,axis=-1)
                one_hot_label = tf.reshape(one_hot_label,[-1,int(global_input_Y_label_shape[-1])])
                GeneratedY = self.G(single_seed,one_hot_label,training=False)
    ...
    def get_seed(self):
        ...
        for i in range(100):
            ...
            one_hot_label = tf.one_hot(label,depth=int(global_input_Y_label_shape[-1]),on_value=1.0,off_value=0.0,axis=-1)
            one_hot_label = tf.reshape(one_hot_label,[-1,int(global_input_Y_label_shape[-1])])
            self.seed.append((seed,one_hot_label))

    def wirte_summary(self,step,seed,G,G_loss,D_loss,out_path):
        plt.figure(figsize=(10,10))#图片大一点才可以承载像素
        for i,(single_seed,label)in enumerate(seed):
            GeneratedY = G(single_seed,label,training=False)
            plt.subplot(10,10,(i+1))
            plt.imshow(GeneratedY[0,:,:,0],cmap='gray')
            plt.axis('off')
...
if __name__ == "__main__":
    ...
    print(list(y.numpy()))
    print(tf.one_hot(2,depth=10,on_value=1.0,off_value=0.0,axis=-1))