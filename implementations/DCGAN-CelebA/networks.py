...
class Generator(tf.keras.Model):
    def __init__(self,**kwargs):
        ...
        self.block_list = []
        base_v = 64
        self.block_list.append(blocks.vec2img(units=7*5*base_v*8,**kwargs))
        self.block_list.append(blocks.uk_5s1_2D(filters=base_v*8,**kwargs))
        self.block_list.append(blocks.uk_5s2_2D(filters=base_v*4,**kwargs))
        self.block_list.append(blocks.uk_5s2_2D(filters=base_v*2,**kwargs))
        self.block_list.append(blocks.uk_5s2_2D(filters=base_v,**kwargs))
        self.block_list.append(blocks.lats_up(filters=3,activation="sigmoid",**kwargs))
    ...
    def call(self,x,training=True):
        ...
        for i,item in enumerate(self.block_list):
            if (i+1) == 1:
                ...
                in_put = tf.reshape(out_put,shape=[-1,7,5,512])
            else:
                out_put = item(in_put,training=training)
                in_put = out_put
...
class Discriminator(tf.keras.Model):
    def __init__(self,use_sigmoid=True,**kwargs):
        ...
        self.block_list.append(blocks.ckd_5s2_2D(filters=base_v*4,**kwargs))#1
        self.block_list.append(blocks.ckd_5s2_2D(filters=base_v*8,**kwargs))
        if use_sigmoid:
...
if __name__ == '__main__':
    ...
    d.build(input_shape=[None,28,20,3])
    z2 = tf.random.normal(shape=[128,28,20,3])
    print(d(z2).shape)
    print(d(z2).dtype)
